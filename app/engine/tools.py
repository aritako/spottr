...
class Tools:
    ...
    def estimated_one_rep_max(self, weight: float, reps: int) -> float:
        """
    ...
    def set_tonnage(self, weight: float, reps: int) -> float:
        """
    ...
    def workout_tonnage(self, sets: list[Set]) -> float:
        """
        ...
        if not sets:
            return 0.0
        return sum(self.set_tonnage(set_["weight"], set_["reps"]) for set_ in sets)

    def weekly_tonnage(self, entries: list[DatedTonnage]) -> list[tuple[str, float]]:
        """
    ...
    def weekly_tonnage_by_exercise(
        ...
        for exercise, dated_tonnage in exercise_bucket.items():
            final_bucket[exercise] = self.weekly_tonnage(dated_tonnage)
